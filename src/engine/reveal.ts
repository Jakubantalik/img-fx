 ...
 *
 * The shader luminance / shader-color sampling reads via gl.readPixels into a
 * small 100×100 buffer (image.html `MASK_SIZE`); the dotMode is temporarily
...
function ensureScratch(state: RevealInternals): void {
  if (!state.sampleCanvas) {
    ...
    state.sampleCanvas.width = MASK_SIZE;
    state.sampleCanvas.height = MASK_SIZE;
  ...
  for (let r = 0; r < MASK_SIZE; r++) {
    for (let c = 0; c < MASK_SIZE; c++) {
      ...
      if (useShader && sampleData) {
        ...
        val = getMaskValue(maskShape, r, c, MASK_SIZE);
      }
      let a = (val - threshold) / softBand;
      if (a < 0) a = 0;
      else if (a > 1) a = 1;
      a = a * a * (3 - 2 * a);
      const idx = (r * MASK_SIZE + c) * 4;
      md[idx] = 255;
      md[idx + 1] = 255;
      md[idx + 2] = 255;
